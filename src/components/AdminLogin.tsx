...
export const AdminLogin: React.FC<AdminLoginProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // OAuth redirects the browser — no success callback needed here.
    } catch (error) {
      setSocialLoading('');
      const message = error instanceof Error ? error.message : 'Unable to start Google sign-in.';
      setErrorMsg(/provider is not enabled|unsupported provider/i.test(message)
        ? 'Google sign-in is not enabled. Enable it in Supabase Dashboard > Authentication > Providers, then try again.'
        : message);
    }
  };
          ...
          <div className="p-6 sm:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
              ...
              <div className="relative py-2">
                <div className="border-t border-[#4a3e34]" />
                <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#1f1a16] px-3 text-[10px] uppercase tracking-wider text-[#857668]">
                  Or continue with
                </span>
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-white hover:bg-gray-50 disabled:opacity-60 disabled:cursor-not-allowed text-[#3c4043] text-xs font-semibold flex items-center justify-center gap-2.5 transition-all active:scale-[0.98]"
              >
